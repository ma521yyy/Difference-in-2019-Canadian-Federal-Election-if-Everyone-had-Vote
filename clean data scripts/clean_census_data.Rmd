...
```{r}
#Create cell counts:
cell_counts <- census %>% group_by(province, sex, as.factor(age), employed, bornin_canada, edu) %>% count() %>% rename(age = "as.factor(age)")
#Create a .csv file
write.csv(cell_counts, "census_cleaned.csv")
```
